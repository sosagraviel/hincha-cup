 ...
 *
 * Implements a five-layer error recovery architecture:
 ...
 * - Layer 5: Infinite retry loop detection
 *
 * Usage:
...
// Store retry history for detecting stuck loops
// Map structure: "context:errorHash" -> [timestamps]
...
/**
 * Clear retry history for a specific context
 */
 ...
 * @param {string} options.context - Context for stuck retry detection (e.g., 'coverage-retry')
 * @param {Function} options.onRetry - Callback called on each retry (receives attempt, error, delay)
 * @param {Function} options.onStuck - Callback called when stuck retry loop detected (receives error, stuckResult)
...
async function retryWithBackoff(fn, options = {}) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      // Check for stuck retry loop
      const stuckResult = detectStuckRetries(error, context);

      if (stuckResult.isStuck) {
        console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        console.error('  ⚠️  INFINITE RETRY LOOP DETECTED');
        console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        console.error('');
        console.error(`Context: ${context}`);
        console.error(`Error hash: ${stuckResult.errorHash}`);
        console.error(`Attempts: ${stuckResult.attempts}`);
        console.error(`Time span: ${stuckResult.timeSpan}s`);
        console.error('');
        console.error(stuckResult.suggestion);
        console.error('');

        if (onStuck) {
          return await onStuck(error, stuckResult);
        } else {
          throw new Error(`Stuck in retry loop: ${error.message}`);
        }
      }
