...
Run this pipeline for each target repo. Before Step 1, set `$REPO_PATH`/`$REPO_BASENAME` for the repo and run the Artifact Paths snippet to define `$ARTIFACTS_DIR` and `$SCANNER_OUT` (always under the workspace-root base, never inside `$REPO_PATH`).
...
When `--aggregate` is set and more than one repo was scanned, read `agents/cross-repo-aggregator.md` and follow it inline. Resolve `$ARTIFACTS_BASE` (see Artifact Paths) — same base the per-repo scans used; their `security-results.json` files live under `$ARTIFACTS_BASE/security/*/`. Write outputs to `$ARTIFACTS_BASE/security/cross-repo-summary.{json,md}`.
