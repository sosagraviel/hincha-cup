...
Codex does not spawn subagents programmatically. Execute this skill yourself by following the phases below and invoking the shell wrapper directly.
...
3. Check that `AGENTS.md` exists at `docs/llm-wiki/AGENTS.md` (Codex schema document).
...
- If `docs/llm-wiki/wiki/` does not exist, the lint service scans zero pages and returns a clean report.
- If the graph DB is absent, graph-version and graph-commit mismatch checks are skipped.
- If the MCP server is unreachable, the dispatch-blind check is skipped with a single warning in the stats.
- If the contradiction check fails (Claude CLI not available), that check is skipped silently.
