...
If `project-context` is not available, read `{{CONFIG_DIR}}/framework-config.json` and `{{CONFIG_DIR}}/{{INSTRUCTION_FILE}}` directly to extract the same information.
...
Depending on what the project uses (check `{{CONFIG_DIR}}/framework-config.json` or `{{CONFIG_DIR}}/{{INSTRUCTION_FILE}}`):
...
1. Output path: `{{CONFIG_DIR}}/triage-reports/YYYY-MM-DD-{affected-service}-triage.md`
...
```
✅ Triage report saved to: {{CONFIG_DIR}}/triage-reports/YYYY-MM-DD-{service}-triage.md
```
