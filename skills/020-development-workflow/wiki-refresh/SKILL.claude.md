...
- If the code graph update fails, warn the user and continue with a stale graph. The wiki pages will still be refreshed from the existing graph data.
- If the agent invoker fails for a specific page, report which page failed and continue with the rest.
- If structural lint failures block the .state.json write, tell the user which violations need fixing and how to re-run.
