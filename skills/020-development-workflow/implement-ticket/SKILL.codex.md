...
**ALL artifacts MUST be saved under the workspace-root temp dir:**
...
`ARTIFACTS_DIR` MUST be an **absolute** path anchored at the workspace root — never a bare relative `{{TEMP_DIR}}/...`, which resolves against the current working directory and leaks into a child repo when `cwd` has drifted. Set and export it once at the start:
```bash
...
`project_path()` resolves the workspace root independent of `cwd`, so `ARTIFACTS_DIR` stays under the workspace root even when a later phase operates on a child repo via `git -C` or after a `cd`. When invoking sub-skills, ALWAYS pass `--artifacts-dir "$ARTIFACTS_DIR"`. This keeps artifacts out of PRs (locally ignored in every repo by Phase 0), keeps paths consistent, and avoids polluting version control.
...
```
source "{{CONFIG_DIR}}/scripts/lib/resolve-paths.sh"
PROGRESS_FILE="$(project_path)/{{TEMP_DIR}}/tickets/$TICKET_ID/progress.jsonl"
...
- Run `/pr-reviewer --pr-url <URL> --jira-key <TICKET_ID> --mode automated --artifacts-dir "$ARTIFACTS_DIR" [--repos <abs-repo-path>]`.
- Run `/security-review --pr-url <URL> --jira-key <TICKET_ID> --artifacts-dir "$ARTIFACTS_DIR" [--repos <abs-repo-path>] [--baseline <prior-findings.json>]`.
...
- `/pr-reviewer --aggregate --jira-key <TICKET_ID> --artifacts-dir "$ARTIFACTS_DIR"` → writes `$ARTIFACTS_DIR/pr/cross-repo-summary.{json,md}` (cross-repo concerns + merge order).
- `/security-review --aggregate --jira-key <TICKET_ID> --artifacts-dir "$ARTIFACTS_DIR"` → writes `$ARTIFACTS_DIR/security/cross-repo-summary.{json,md}` (cross-cutting security concerns + dependency-ordered remediation).
...
- Archive artifacts (`tar` the `$ARTIFACTS_DIR` into `$(dirname "$ARTIFACTS_DIR")/artifacts.tar.gz` — anchored to the absolute `$ARTIFACTS_DIR`, never a bare relative path).
...
- For other git hosts: the provider's native CLI configured and authenticated against every affected remote.
