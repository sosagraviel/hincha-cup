...
The runtime consumers (`apply-inspection-postfill.ts`, the `language-config-summary` prompt script, manifest / lock-file / runtime-version tables, etc.) are auto-derived from the registry. There is no other "language list" anywhere in the codebase.
