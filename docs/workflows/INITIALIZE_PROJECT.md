...
- **`framework-config.json`.** The committed config is deterministic — the volatile `project_metadata` block (`project_path`, `initialization_hash`, `last_analysis`) and the per-resource `last_sync` timestamps were dropped. They changed on every run, produced noisy diffs and merge conflicts, and nothing read them. A single top-level `resource_state.last_sync` is kept (written only when sync changes resource state; omitted by the init writer); the stable `stack_profile` / `resource_state` / version fields stay committed.
