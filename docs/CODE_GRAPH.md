...
3. **Claude CLI Stop hook / Codex CLI in-process extractor verify usage and write a sidecar.** Two equivalent paths produce the same `<sessionId>.graph-tool-uses.json` sidecar shape:
   - **Claude CLI:** `phase1/shared/hooks/validate-analyzer-json.hook.ts` runs as a Stop hook, reads `transcript.jsonl` from the analyzer's session directory, counts `tool_use` events whose `name` matches `^mcp__code_graph__`, and writes the sidecar to `~/.claude/projects/<slug>/<sessionId>.graph-tool-uses.json` next to the transcript. If the analyzer is graph-required and the count is zero, the hook BLOCKS the agent (exit 2) with retry-feedback naming the available tools.
   - **Codex CLI:** Codex has no Stop hook (per OpenAI's hook docs only `PreToolUse` is supported), so `codex-cli-agent-impl.ts` does the same job in-process after `runCodex` returns. It uses `locateCodexRollout` to find the rollout JSONL the CLI wrote into `~/.codex/sessions/...`, runs the pure `extractGraphToolUsesFromCodexJsonl` extractor over it, and writes the sidecar to a framework-owned path: `<projectPath>/.codex-temp/initialize-project/graph-tool-uses/<sessionId>.graph-tool-uses.json`. Codex sessions don't get the BLOCKING behaviour Claude does — Codex internally retries via `--json` event-stream feedback, so the Codex path is observability-only.

4. **The Phase 1 orchestration node overwrites `graph_queries_used` from the sidecar via a provider-specific loader.** `phase1/shared/graph-tool-usage.ts` exposes `loadClaudeSidecar` and `loadCodexSidecar`; the analyzer node picks the right one via `getSidecarLoaderForProvider(getActiveProvider())` and passes it to `applyGraphToolUsageFromSidecar(...)`. The agent-supplied value is discarded unconditionally — the field is no longer agent-owned. If the sidecar is missing or malformed, the helper logs a warning and forces `graph_queries_used: []` (honest empty array beats fabricated list).

5. **The Stop hook / Codex extractor also count tool-result overflows and the orchestration surfaces them.** Each call whose result starts with the `Error: result (NNN characters) exceeds maximum allowed tokens. Output has been saved to /Users/.../tool-results/...txt` sentinel is recorded in the sidecar's `overflows: [{tool, callIndex}]` array. The orchestration node stamps `graph_overflow_count` and `graph_overflow_tools` onto the persisted analyzer JSON, and Phase 2 consolidation emits a `WARN` line for any analyzer with `graph_overflow_count > 0`. This makes silently-degraded runs impossible to ignore — for both providers.
