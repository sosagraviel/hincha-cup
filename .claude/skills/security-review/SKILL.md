...
**Single-repo:**
```
.claude-temp/artifacts/<JIRA_KEY>/security/
  sarif.json
...
```
.claude-temp/artifacts/<JIRA_KEY>/security/<repo-basename>/
  sarif.json
  security-results.json
  security-report.md
  scanner-outputs/
...
```
.claude-temp/artifacts/<JIRA_KEY>/security/cross-repo-summary.json
.claude-temp/artifacts/<JIRA_KEY>/security/cross-repo-summary.md
```
...
Both files are written to `.claude-temp/artifacts/<JIRA_KEY>/security/`.
  ...
  "repos": [
    {
      ...
      "minorCount": 1,
      "overallStatus": "FAIL",
      "sarifPath": ".claude-temp/artifacts/PROJ-123/security/my-service/sarif.json"
    }
  ],
