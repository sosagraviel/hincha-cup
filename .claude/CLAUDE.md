...
| MCP user-questions integration | `docs/mcp-user-questions/` | One directory, all artifacts: `README.md` (manual install guide), `*.sh` (hook scripts shipped for engineer-driven install), `fixtures/*.json` (expected payload shapes diffed by `scripts/verify-mcp-payload.sh`). Scripts are self-contained — no framework path coupling. |
...
- **MCP-first user prompts**: skills that need engineer input must gate on `QAF_ASK_USER_MCP_TOOL` when available; provider-native prompting (`AskUserQuestion` / `read -p`) is fallback-only behavior when the MCP tool is unavailable.
