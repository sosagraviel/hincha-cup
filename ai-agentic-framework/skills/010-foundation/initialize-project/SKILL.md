...
# Helper function to copy skill with full directory structure
copy_skill() {
  local skill_path="$1"
  local skill_name=$(basename "$skill_path")
  if [ -d "$skill_path" ]; then
    mkdir -p ".claude/.tmp/skills/$skill_name"
    cp -r "$skill_path"/* ".claude/.tmp/skills/$skill_name/"
    echo "  ✓ $skill_name"
  else
...
if [ -d .claude/.tmp/skills ]; then
  # Remove old skills except project-context
  for dir in .claude/skills/*/; do
    skill_name=$(basename "$dir")
    if [ "$skill_name" != "project-context" ]; then
      rm -rf "$dir"
    fi
  done

  cp -r .claude/.tmp/skills/* .claude/skills/
  echo "  ✓ Skills committed"
fi

# Replace agents
if [ -d .claude/.tmp/agents ]; then
  rm -rf .claude/agents
  mv .claude/.tmp/agents .claude/agents
...
```bash
total_lines=$(wc -l .claude/CLAUDE.md .claude/skills/project-context/SKILL.md .claude/agents/*.md | tail -1 | awk '{print $1}')
skill_count=$(ls -1 .claude/skills/ | wc -l)
agent_count=$(ls -1 .claude/agents/ | wc -l)
```
