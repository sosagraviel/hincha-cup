...
async function detectAtlassianMCP(projectPath) {
  ...
  if (claudeMd && (claudeMd.includes('Jira') || claudeMd.includes('Confluence'))) {
    needed = true;
    reasons.push('Jira/Confluence mentioned in CLAUDE.md');
  }

  // Check for Jira skills
  const hasJiraSkill = await directoryExists(projectPath, '.claude/skills/jira') ||
  ...
  return {
    ...
    required_credentials: ['ATLASSIAN_CLOUD_ID', 'ATLASSIAN_USER_EMAIL', 'ATLASSIAN_API_TOKEN'],
    setup_instructions: {
      ...
      test_command: 'curl -u "$ATLASSIAN_USER_EMAIL:$ATLASSIAN_API_TOKEN" "https://api.atlassian.com/oauth/token/accessible-resources"'
    }
  };
}
...
async function detectGitHubMCP(projectPath) {
  ...
  if (await directoryExists(projectPath, '.claude/skills/mastering-github-agent-skill')) {
    needed = true;
  ...
  return {
    ...
    setup_instructions: {
      ...
      test_command: 'curl -H "Authorization: token $GITHUB_PERSONAL_ACCESS_TOKEN" https://api.github.com/user'
    }
  };
}
...
async function detectPlaywrightMCP(projectPath, stackProfile) {
  ...
  if (await fileExists(projectPath, 'playwright.config.ts') || await fileExists(projectPath, 'playwright.config.js')) {
    needed = true;
...
async function detectNotionMCP(projectPath) {
  ...
  if (await directoryExists(projectPath, '.claude/skills/notion-document-manager')) {
    needed = true;
  ...
  if (envFile && (envFile.includes('NOTION_API_KEY') || envFile.includes('NOTION_TOKEN'))) {
    needed = true;
...
async function detectGoogleSheetsMCP(projectPath) {
  ...
  // Check CLAUDE.md
  const claudeMd = await readFile(projectPath, '.claude/CLAUDE.md');
  if (claudeMd && (claudeMd.includes('Google Sheets') || claudeMd.includes('spreadsheet'))) {
    needed = true;
...
async function detectPencilMCP(projectPath) {
  ...
  const claudeMd = await readFile(projectPath, '.claude/CLAUDE.md');
  if (claudeMd && (claudeMd.includes('design') || claudeMd.includes('Figma') || claudeMd.includes('UI') || claudeMd.includes('mockup'))) {
    needed = true;
...
async function generateMCPFiles(projectPath, mcpResults) {
  ...
  if (mcpResults.needs_setup.length > 0) {
    ...
    setupMd += 'The following MCPs need credentials before they can be used:\n\n';
  ...
  if (mcpResults.not_needed.length > 0) {
    ...
    setupMd += 'The following MCPs were not configured (not detected as needed):\n\n';
  ...
  setupMd += '3. Start using MCPs in Claude Code: `List my Jira issues` or `Show my GitHub PRs`\n';
