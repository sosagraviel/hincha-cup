...
check_secrets() {
  ...
  for pattern in "${secret_patterns[@]}"; do
    if grep -r -E "$pattern" ai-agentic-framework/ --exclude-dir=node_modules --exclude-dir=.git 2>/dev/null | grep -v "security-check.sh" | grep -q .; then
      critical "Potential hardcoded secret found: $pattern"
      grep -r -E "$pattern" ai-agentic-framework/ --exclude-dir=node_modules --exclude-dir=.git 2>/dev/null | grep -v "security-check.sh" | head -3
      found_secrets=true
    fi
  done
  ...
  if grep -r "process\.env\." ai-agentic-framework/utils/ 2>/dev/null | grep -q .; then
    local has_validation=false
    if grep -r "throw.*Error.*environment" ai-agentic-framework/utils/ 2>/dev/null | grep -q .; then
      has_validation=true
    fi
...
check_input_validation() {
  ...
  for pattern in "${unsafe_patterns[@]}"; do
    if grep -r -E "$pattern" ai-agentic-framework/utils/ 2>/dev/null | grep -q .; then
      fail "Potential command injection: $pattern"
      grep -r -E "$pattern" ai-agentic-framework/utils/ 2>/dev/null | head -2
      found_unsafe=true
    fi
  done
  ...
  if grep -r "path\.join.*\.\." ai-agentic-framework/utils/ 2>/dev/null | grep -q .; then
    warn "Potential path traversal with '..' found"
    grep -r "path\.join.*\.\." ai-agentic-framework/utils/ 2>/dev/null | head -2
  else
    pass "No path traversal patterns found"
  fi

  # Check ticket key validation
  if grep -r "ticketKey" ai-agentic-framework/utils/ 2>/dev/null | grep -q .; then
    if grep -r "PROJ-[0-9]" ai-agentic-framework/utils/ 2>/dev/null | grep -q .; then
...
check_file_permissions() {
  ...
  local scripts=(
    "ai-agentic-framework/utils/security-check.sh"
    "ai-agentic-framework/tests/test-autonomous-workflow.sh"
  )
...
check_code_injection() {
  ...
  if grep -r "eval(" ai-agentic-framework/utils/ 2>/dev/null | grep -v "security-check.sh" | grep -q .; then
    critical "eval() usage found (code injection risk)"
    grep -r "eval(" ai-agentic-framework/utils/ 2>/dev/null | grep -v "security-check.sh" | head -3
  else
    pass "No eval() usage found"
  fi

  # Check for Function constructor
  if grep -r "new Function(" ai-agentic-framework/utils/ 2>/dev/null | grep -q .; then
    fail "Function constructor found (code injection risk)"
    grep -r "new Function(" ai-agentic-framework/utils/ 2>/dev/null | head -2
  else
    pass "No Function constructor usage found"
  fi

  # Check for unsafe JSON parsing
  if grep -r "JSON\.parse(.*req\." ai-agentic-framework/ 2>/dev/null | grep -q .; then
...
check_logging() {
  ...
  for pattern in "${sensitive_log_patterns[@]}"; do
    if grep -r -i -E "$pattern" ai-agentic-framework/utils/ 2>/dev/null | grep -v "security-check.sh" | grep -q .; then
      warn "Potential sensitive data logging: $pattern"
      grep -r -i -E "$pattern" ai-agentic-framework/utils/ 2>/dev/null | grep -v "security-check.sh" | head -2
...
check_access_control() {
  ...
  if grep -r "process\.getuid" ai-agentic-framework/utils/ 2>/dev/null | grep -q .; then
  ...
  if grep -r "sudo" ai-agentic-framework/ 2>/dev/null | grep -v "security-check.sh" | grep -q .; then
    warn "sudo usage found (potential privilege escalation)"
    grep -r "sudo" ai-agentic-framework/ 2>/dev/null | grep -v "security-check.sh" | head -2
  else
