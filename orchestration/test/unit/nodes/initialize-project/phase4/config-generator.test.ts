...
describe("config-generator", () => {
  ...
  describe("generateFrameworkConfig", () => {
    ...
    it("should handle testing frameworks", () => {
      ...
      expect(config.stack_profile.services[0].testing?.unit?.framework).toBe(
        "vitest",
      );
      expect(config.stack_profile.services[0].testing?.e2e?.framework).toBe(
        "playwright",
      );
      expect(config.stack_profile.services[1].testing?.unit?.framework).toBe(
        "pytest",
      );
    });
    ...
    it("should handle infrastructure data", () => {
      ...
      expect(config.stack_profile.infrastructure).toEqual([
        "docker",
        "kubernetes",
        "aws",
      ]);
    ...
    it("should handle complex multi-workspace monorepo", () => {
      ...
      const languages = new Set(
        config.stack_profile.services.map((s) => s.language),
      );
      expect(Array.from(languages).sort()).toEqual([
        "go",
        "python",
        "typescript",
      ]);
    ...
    it("should handle mobile frameworks", () => {
      ...
      const mobileServices = config.stack_profile.services.filter(
        (s) => s.type === "mobile",
      );
      expect(mobileServices).toHaveLength(2);
      expect(mobileServices.map((s) => s.frameworks.main)).toEqual([
        "react-native",
        "flutter",
      ]);
    });
