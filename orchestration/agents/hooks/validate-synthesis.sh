...
validate_synthesis() {
  ...
  if [ "${#issues[@]}" -gt 0 ]; then
    echo ""
    echo "❌ SYNTHESIS OUTPUT VALIDATION FAILED - Fix these issues:"
    echo ""
    # Number the issues
    local i=1
    ...
    echo ""
    echo "📋 REQUIRED MARKDOWN FORMAT:"
    echo ""
    echo "# CLAUDE.md Content"
    echo ""
    echo "[Content requirements:]"
    echo "  - 30-250 lines of markdown"
    echo "  - Must include: ## Tech Stack"
    echo "  - Must include: ## File Placement Guide"
    echo "  - Must include: ## Essential Commands"
    echo ""
    echo "---"
    echo ""
    ...
    echo ""
    echo "---"
    echo "[YAML frontmatter with skill metadata]"
    echo "---"
    echo ""
    echo "[Content requirements:]"
    echo "  - 50-600 lines of markdown"
    echo "  - Must start with YAML frontmatter (---)  "
    echo "  - Contains comprehensive project context"
    echo ""
    echo "⚠️ CRITICAL REMINDERS:"
    echo "  1. Output MARKDOWN text only, NOT a JSON object"
    echo "  2. Include ALL required sections and subsections"
    echo "  3. Use exactly the section headers shown above"
    echo "  4. Ensure content lengths are within specified ranges"
    echo ""
    echo "Please output the corrected markdown now."
    echo ""
    return 1
  fi
