...
export const ServiceEnvironmentSchema = z.object({
  ...
  deployment_target: z.string().optional().describe('Deployment target (e.g., "GCP Functions", "AWS Lambda", "Vercel")'),
...
export const ServiceFrameworksSchema = z.object({
  main: z.string().optional().describe('Main framework (e.g., "NestJS 11", "React 19", "Flask 3.0")'),
  orm: z.string().optional().describe('ORM framework (e.g., "TypeORM 0.3", "SQLAlchemy 2.0")'),
  ui: z.string().optional().describe('UI library/framework (e.g., "shadcn/ui", "Material-UI", "Tailwind CSS v4")'),
  testing: z.string().optional().describe('Primary testing framework (e.g., "Jest", "Pytest", "Playwright")'),
  additional: z.array(z.string()).optional().describe('Additional frameworks (e.g., ["Socket.IO", "BullMQ"])'),
});
...
export const ServiceSchema = z.object({
  ...
  databases: z.array(ServiceDatabaseSchema).optional().describe('Databases used by this service (discovered from dependencies)'),
  ...
  manifest_file: z.string().optional().describe('Path to manifest file (DISCOVERED dynamically, e.g., "src/api/package.json")'),
});
...
export const StackProfileSchema = z.object({
  ...
  services: z.array(ServiceSchema).min(1).describe(
  ...
  is_monorepo: z.boolean().describe('Whether the project is a monorepo (multiple packages/services in one repository)'),
  workspace_tool: z.string().optional().describe('Workspace tool detected (e.g., "pnpm workspaces", "Nx", "Turborepo", "Lerna")'),
  package_manager: z.string().optional().describe('Root-level package manager (e.g., "pnpm", "npm", "yarn")'),
  infrastructure: z.array(z.string()).optional().describe('Infrastructure tools detected (e.g., ["Docker", "Kubernetes", "Terraform"])'),

  // File counts (optional metadata for context)
  file_counts: z.object({
    total: z.number().describe('Total number of files in repository'),
    by_language: z.record(z.string(), z.number()).describe('File counts grouped by language extension'),
  }).optional().describe('File statistics for the repository'),
}).refine(
  (data) => {
...
export function getAllTestingFrameworks(stackProfile: StackProfile): string[] {
  ...
  for (const service of stackProfile.services) {
    ...
    if (service.testing?.integration?.framework) frameworks.add(service.testing.integration.framework);
