...
export async function validationNode(
  ...
  try {
    ...
    if (!state.project_context_path || !existsSync(state.project_context_path)) {
      validationErrors.push('project-context/SKILL.md not found');
    } else {
      const projectContextContent = readFileSync(state.project_context_path, 'utf-8');
    ...
    if (!state.framework_config_path || !existsSync(state.framework_config_path)) {
      ...
      try {
        ...
        if (!config.project_metadata) {
          validationErrors.push('framework-config.json missing project_metadata');
        }
    ...
    if (!existsSync(agentsDir)) {
      ...
      // Check minimum agent count (at least 2: planner + 1 implementer)
      if (agentCount < 2) {
      ...
      if (state.framework_config_path && existsSync(state.framework_config_path)) {
        try {
          ...
          if (stackProfile && stackProfile.file_counts) {
            // Get languages with significant file counts (>10 files)
            const significantLanguages = Object.entries(stackProfile.file_counts)
            ...
            for (const lang of significantLanguages) {
              const hasImplementer = agentFiles.some(f =>
          ...
          validationWarnings.push(`Could not validate multi-stack coverage: ${(error as Error).message}`);
        }
      }
    }
